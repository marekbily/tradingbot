...
def _to_utc_timestamp(time_text: str | None, source_timezone: str) -> str | None:
	...
	local_dt = pd.to_datetime(time_text, format="%d/%m/%Y %H:%M", errors="coerce")
	if pd.isna(local_dt):
		local_dt = pd.to_datetime(time_text, errors="coerce")
	if pd.isna(local_dt):
		return None

	tzinfo = _infer_timezone(source_timezone)
	if local_dt.tzinfo is None:
		local_dt = local_dt.to_pydatetime().replace(tzinfo=tzinfo)
	else:
		local_dt = local_dt.to_pydatetime().astimezone(tzinfo)
...
def _write_csv(records: list[dict[str, Any]], output_path: Path) -> Path:
	...
	if "raw" in frame.columns:
		frame["raw"] = frame["raw"].apply(lambda value: json.dumps(value, ensure_ascii=False, default=str))
...
def download_calendar(
	...
	if output_format in {"json", "both"}:
		json_path = _write_json(records, output_dir_path / f"{base_name}.json")
	if output_format in {"csv", "both"}:
		csv_path = _write_csv(records, output_dir_path / f"{base_name}.csv")
...
def build_arg_parser() -> argparse.ArgumentParser:
	...
	parser.add_argument("--source", choices=sorted(SOURCE_SCRAPERS), default="forex")
...
def main() -> int:
	...
	logger.info("Downloaded %s rows", result.row_count)
	if result.json_path:
		logger.info("JSON saved to %s", result.json_path)
	if result.csv_path:
		logger.info("CSV saved to %s", result.csv_path)
	return 0
